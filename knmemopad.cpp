...
#include "brain-essential/kncedlg-0.10/kncedlg.h"
#include "brain-essential/knceutil-0.12/knceutil.h"
...
static INT ConvertCRLF(tstring &strRet /*Store results*/, tstring strString /*Target String*/,
                       tstring strCRLF /*Replace to this string*/);
  ...
  switch (msg) {
    ...
    case WM_COMMAND: {
      ...
      switch (id) {
        ...
        case IDM_ABOUT:
          MessageBox(hWnd,
                     _T("KN MemoPad improved by watamario15 v0.12 rev7\n\n")
  ...
  switch (msg) {
    ...
    case WM_KEYDOWN: {
      ...
      if (data->isSelectMode) {
        if (find(data->selectableKeys.begin(), data->selectableKeys.end(), keyCode) == data->selectableKeys.end()) {
          return 0;
        }
      }
...
static void onActivate(HWND hWnd, int act) {
  ...
  if (act == WA_ACTIVE || act == WA_CLICKACTIVE) {
    ...
    if (data->isSelectMode) onEditEndSelect(hWnd);
  }
}
...
static void onSize(HWND hWnd, int width, int height) {
  ...
  if (data->isStatusBar) {
    ...
    data->hStatusBar =
        CreateStatusWindow(WS_CHILD | WS_VISIBLE | CCS_BOTTOM, _T("Line: 1 Column: 1"), hWnd, IDC_STATUS_BAR);
    ...
    if (data->charset == 0)
      SendMessage(data->hStatusBar, SB_SETTEXT, 2, (LPARAM) _T("Shift_JIS (MS932, ANSI_JP)"));
    else if (data->charset == 1) {
      if (data->AddBOM)
        SendMessage(data->hStatusBar, SB_SETTEXT, 2, (LPARAM) _T("UTF-8 BOM (Unicode)"));
      else
        SendMessage(data->hStatusBar, SB_SETTEXT, 2, (LPARAM) _T("UTF-8 (Unicode)"));
    } else if (data->charset == 2)
      SendMessage(data->hStatusBar, SB_SETTEXT, 2, (LPARAM) _T("UTF-16 LE (Unicode)"));
    else if (data->charset == 3)
      SendMessage(data->hStatusBar, SB_SETTEXT, 2, (LPARAM) _T("CP1252 (ANSI_EN)"));
...
static void onInitMenuPopup(HWND hWnd) {
  ...
  CheckMenuRadioItem(hMenu, IDM_TOOLS_CHARSET_MS932, IDM_TOOLS_CHARSET_CP1252, IDM_TOOLS_CHARSET_MS932 + data->charset,
                     MF_BYCOMMAND);
...
static void openFile(HWND hWnd, const tstring &fileName, bool isReload) {
  ...
  if (data->csAutoDetect && !isReload) {
    ...
    if (mbBuff[0] == '\xEF' && mbBuff[1] == '\xBB' && mbBuff[2] == '\xBF')
      data->charset = 1;
    else if (mbBuff[0] == '\xFF' && mbBuff[1] == '\xFE')
      data->charset = 2;
    else if (data->charset == 2)
      data->charset = 0;
  ...
  if (data->charset == 0) {
    ...
    if (Buff[0] == L'\xFEFF')
      data->crlf = ConvertCRLF(tstrBuff, &Buff[1], _T("\r\n"));
    else
      data->crlf = ConvertCRLF(tstrBuff, Buff, _T("\r\n"));
  } else if (data->charset == 3) {
    ReadFile(hFile, mbBuff, dwsize, &dwtemp, NULL);
...
static bool saveFile(HWND hWnd, bool isOverwrite) {
  ...
  if (data->charset == 1 && data->AddBOM)
    WriteFile(hFile, "\xEF\xBB\xBF", strlen("\xEF\xBB\xBF") * sizeof(char), &dwtemp, NULL);
  ...
  if (data->crlf == 1)
    ConvertCRLF(tstrBuff, editBuff, _T("\n"));
  else if (data->crlf == 2)
    ConvertCRLF(tstrBuff, editBuff, _T("\r"));
  if (data->charset == 0) {
    ret = WideCharToMultiByte(932, 0, (data->crlf == 0 ? editBuff : tstrBuff.c_str()), -1, mbBuff, MAX_EDIT_BUFFER * 2,
                              NULL, NULL);
    WriteFile(hFile, mbBuff, ret - 1, &dwtemp, NULL);
  } else if (data->charset == 1) {
    ret = WideCharToMultiByte(65001, 0, (data->crlf == 0 ? editBuff : tstrBuff.c_str()), -1, mbBuff,
                              MAX_EDIT_BUFFER * 2, NULL, NULL);
    WriteFile(hFile, mbBuff, ret - 1, &dwtemp, NULL);
  } else if (data->charset == 2) {
    WriteFile(hFile, (data->crlf == 0 ? editBuff : tstrBuff.c_str()),
              _tcslen((data->crlf == 0 ? editBuff : tstrBuff.c_str())) * sizeof(TCHAR), &dwtemp, NULL);
  } else if (data->charset == 3) {
    ret = WideCharToMultiByte(1252, 0, (data->crlf == 0 ? editBuff : tstrBuff.c_str()), -1, mbBuff, MAX_EDIT_BUFFER * 2,
                              NULL, NULL);
    WriteFile(hFile, mbBuff, ret - 1, &dwtemp, NULL);
  }
